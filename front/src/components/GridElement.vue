    ...
    computed: {
        getEvent() {
            this.ev = this.filterStore.results.filter((e) => {
                if (this.day != null && this.room != null) {
                    return e.roomId == this.room && (e.startDate.getDay() <= this.day && e.endDate.getDay() >= this.day)
                } else {
                    return false;
                }
            }
        ...
        setGridStyle() {
            ...
            if (this.ev != null && this.day != null && this.room != null) {
                ...
                let color = "background-color:rgba("+this.ev.startDate.getDay()*42.5+","+this.room*23.18+","+((this.room-this.ev.startDate.getDay())+5)*25+", 50%);";
                style += color;
            }
            return style
        },
    ...
    <div class="gridElement" :style="setGridStyle" @click="selectEvent">
        <div v-if="getEvent != null && day !=null && room != null">
            <div class="infoEvent" v-if="getEvent.startDate.getDay() == day">
                ...
                <span style="display:flex;flex-direction:row;width:100%;justify-content:space-between">
