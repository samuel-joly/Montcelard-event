...
interface OffsetInterface { day: number, hour: string };
...
export default defineComponent({
        ...
        setStartHourOffset(value: string, index: number) {
            if (this.gridStore.selected != null) {
                if (index == 0) {
                    ...
                    this.gridStore.selected.startHourOffset = this.fromOffsetToStr(this.startOffsets, this.gridStore.selected.startHour.replace(":", "."));
                }
            }
        },
        setEndHourOffset(value: string, index: number) {
            if (this.gridStore.selected != null) {
                if (index == 0) {
                    ...
                    this.gridStore.selected.endHourOffset = this.fromOffsetToStr(this.endOffsets, this.gridStore.selected.endHour.replace(":", "."));
                }
            }
        },
    },
    computed: {
        getResaDays() {
            if (this.gridStore.selected != null && this.startOffsets.length == 0 && this.endOffsets.length == 0) {
                ...
                startOffs.split(";").map((offset) => {
                    if (this.gridStore.selected != null && offset != "") {
                        this.startOffsets.push(this.fromStrToOffset(Number(this.gridStore.selected.startHour.replace(":", ".")), offset))
                    }
                })
                endOffs.split(";").map((offset) => {
                    if (this.gridStore.selected != null && offset != "") {
                        this.endOffsets.push(this.fromStrToOffset(Number(this.gridStore.selected.endHour.replace(":", ".")), offset))
                    }
                })
                this.resaDays = [];
                this.daySelection = 0;
...
<template>
    <div id="resaHour" class="flex-row just-around" style="width:100%;">
        <div v-for="(_, index) in getResaDays" v-show="(daySelection == index)"
            class="flex-col align-center just-start dayHour">
            <div class="flex-row">
                <label :for="'start_hour_' + index" style="width:4em;">Arrivée</label>
                <input type="time" :name="'start_hour_' + index" :value="getStartHourValue(index)"
                    @input="(v) => { setStartHourOffset((v.target as HTMLInputElement).value, index) }" />
            </div>
            <div class="flex-row">
                <label :for="'end_hour_' + index" style="width:4em;">Départ</label>
                <input type="time" :name="'end_hour_' + index" :value="getEndHourValue(index)"
                    @input="(v) => { setEndHourOffset((v.target as HTMLInputElement).value, index) }" />
            </div>
        </div>
        <div class="flex-col just-end align-start" style="height:100%;">
            <select v-model="daySelection">
                <option v-for="(day, index) in getResaDays" :value="index">{{ day }}
                </option>
            </select>
        </div>
    </div>
