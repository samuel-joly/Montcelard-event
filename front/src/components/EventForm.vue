                    ...
                    <div class="flex-row just-between">
                        <span>
                            <input name="start_date" type="date"
                                :value="filterStore.selected.startDate.toISOString().split('T')[0]" @input="event => {
                    ...
                    <div class="flex-row just-between">
                        <span>
                            <input name="end_date" type="date"
                                :value="filterStore.selected.endDate.toISOString().split('T')[0]" @input="event => {
                                    const newDate = (event.target as HTMLInputElement).valueAsDate;
                                    if (filterStore.selected != null && newDate != null) {
            ...
            <div id="roomInfo">
                <div id="conf">
                    ...
                    <span v-if="filterStore.selected.roomConfiguration == 'Ilots'">
                        ...
                        <div class="roomConfigurationValue" style="display: flex; flex-direction: row">
                            <input class="configuration_opt" type="number"
                ...
                <div id="roomFurnitures">
                    <span>
                        ...
                        <input type="number" id="paperboard" name="paperboard" v-model="filterStore.selected.paperboard" />
                    </span>
                    <span>
                    ...
                    <span id="hostTable">
                        <label :class="[filterStore.selected.hostTable ? 'selectedFurniture' : '']" for="host_table">Table
                            Formateur</label>
                        <input type="checkbox" id="host_table" name="host_table" v-model="filterStore.selected.hostTable" />
                    </span>
                </div>
                <h3>Matériel</h3>
                <div id="furnituresBox">
                    <span class="furnitures">
                        <label :class="[filterStore.selected.pen ? 'selectedFurniture' : '']" for="pen">stylos</label>
                        <input type="checkbox" id="pen" name="pen" v-model="filterStore.selected.pen" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.blocNote ? 'selectedFurniture' : '']"
                            for="bloc_note">bloc-note</label>
                        <input id="bloc_note" type="checkbox" name="bloc_note" v-model="filterStore.selected.blocNote" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.paper ? 'selectedFurniture' : '']" for="paper">A4</label>
                        <input id="paper" type="checkbox" name="paper" v-model="filterStore.selected.paper" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.paperA1 ? 'selectedFurniture' : '']" for="paper_a1">A1</label>
                        <input id="paper_a1" type="checkbox" name="paper_a1" v-model="filterStore.selected.paperA1" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.scissors ? 'selectedFurniture' : '']"
                            for="scissors">ciseau</label>
                        <input id="scissors" type="checkbox" name="scissors" v-model="filterStore.selected.scissors" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.scotch ? 'selectedFurniture' : '']" for="scotch">scotch</label>
                        <input id="scotch" type="checkbox" name="scotch" v-model="filterStore.selected.scotch" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.postIt ? 'selectedFurniture' : '']"
                            for="post_it">post-it</label>
                        <input id="postIt" type="checkbox" name="postIt" v-model="filterStore.selected.postIt" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.postItXl ? 'selectedFurniture' : '']" for="post_it_xl">post-it
                            XL</label>
                        <input id="post_it_xl" type="checkbox" name="post_it_xl" v-model="filterStore.selected.postItXl" />
                    </span>
                    <span class="furnitures">
                        <label :class="[filterStore.selected.gomette ? 'selectedFurniture' : '']"
                            for="gomette">gomette</label>
                        <input id="gomette" type="checkbox" name="gomette" v-model="filterStore.selected.gomette" />
                    </span>
                </div>
                <span>
                    <label for="room_configuration_precision">Précision</label>
                    <textarea name="room_configuration_precision"
