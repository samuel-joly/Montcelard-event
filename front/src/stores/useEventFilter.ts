...
export const useEventFilter = defineStore('eventFilter', {
    ...
    actions: {
        ...
        async fetchEvent() {
            ...
            const weekDateString = firstWeekDay.getFullYear()+"-"+(1+firstWeekDay.getMonth())+"-"+(firstWeekDay.getDate());
            const weekEndDateString = lastWeekDay.getFullYear()+"-"+(1+lastWeekDay.getMonth())+"-"+lastWeekDay.getDate();
            this.results = (await cli.get<Event>(
                new Event(),
                0,
